...
# Count fields
num_fields=`eval "cat results/${APP_NAME}_${cores[1]}cores/stats.out | wc -l"`

# Write fields
fields=""
for (( j=1;j<$num_fields+1;j++)); do
	line=`eval "sed '${j}q;d' results/${APP_NAME}_${cores[1]}cores/stats.out | cut -f1 -d\"=\""`
	fields=${fields}${line},
done
...
for (( i=0;i<$num_sims;i++)); do
	for (( j=1;j<$num_fields+1;j++)); do
		line=`eval "sed '${j}q;d' results/${APP_NAME}_${cores[${i}]}cores/stats.out | cut -f2 -d \"=\" | tr -d \'[[:space:]]\'"`
		data=${data}${line},
	done
